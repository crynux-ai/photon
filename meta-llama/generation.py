...
class Llama:
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if prev_pos == 0:
                print(f"PP {cur_pos} Token {time.time() - start_time:.2f} seconds")
                start_time = time.time()
        ...
        print(f"TG {cur_pos} Token {time.time() - start_time:.2f} seconds")
...
if __name__ == '__main__':
    ...
    res = model.text_completion(["""A photon (from Ancient Greek φῶς, φωτός (phôs, phōtós) 'light') is an
